namespace CodeCounterLibrary
{
    ...
    public class CodeComparison
    {
        ...
        private double runComp(string str1, string str2, SimMetricsApi.AbstractStringMetric jComp)
        {
            
            return jComp.GetSimilarity(str1, str2);
        }

        private bool compareLines(string line1, string line2)
        {
            SimMetricsApi.AbstractStringMetric jaroComparison = new JaroWinkler();
            SimMetricsApi.AbstractStringMetric aAlgoCompare = new JaccardSimilarity();
            SimMetricsApi.AbstractStringMetric bAlgoCompare = new SmithWatermanGotoh();
            ...
            /*
            double jaro = runComp(line1, line2,jaroComparison);
            ...
            if (probableMatch > .80 && probableMatch < 1)
            {
                //We have a match
                matchedLine = true;
            }*/
        ...
        private List<String> compareCode(ref List<string> oldCodeLines, ref List<string> newCodeLines)
        {
           
     
            List<string> modifiedLines = new List<string>();
            //Felt like using old-style loop here.
            for (int i = 0; i < oldCodeLines.Count; i++)
            {
                bool matchedLine = false;
                int j = 0;
                while(j < newCodeLines.Count && ! matchedLine)
                {
                    if (compareLines(oldCodeLines[i], newCodeLines[j]))
                    {
                        matchedLine = true;
        ...
        private void processCode()
        {
            ...
            //First we find all of the modified lines.
            List<string> modifiedLines = compareCode(ref oldCodeLines, ref newCodeLines);
